...
在配置菜单中按 `r` 直接切换并保存 `disable_monitor_retry`（全账号生效，默认 `false`）。设为 `true` 时，监控阶段出错直接报错退出，不进行退避重试；`false` 保留最多 10 次的网络退避重试。也可在 `config.json` 顶层设置该布尔值，下次启动监控时生效。启动阶段始终不退避重试。
