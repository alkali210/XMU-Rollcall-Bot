...
监控内建网络重试，无需额外守护脚本。连接中断、连接/读取超时、响应传输中断，以及 HTTP 408、429、500、502、503、504 会按 5、10、20、40、60 秒退避重试，之后每 60 秒重试。首次请求之外最多重试 10 次，仍失败则提示并退出；期间可按 `Ctrl+C` 退出。成功完成一轮后重置等待时间和重试计数，断线及恢复会显示提示并写入日志。启动时验证缓存会话和登录也使用相同的 10 次重试上限。
