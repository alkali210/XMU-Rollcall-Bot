...
监控内建网络重试，无需额外守护脚本。连接中断、连接/读取超时、响应传输中断、SSL 意外 EOF（连接突然关闭），以及 HTTP 408、429、500、502、503、504 会按 5、10、20、40、60 秒退避重试；之后每 60 秒重试。启动和监控均最多连续重试 10 次（首次请求之外最多再尝试 10 次），耗尽后报错退出；可随时按 `Ctrl+C` 退出。成功完成一轮后重置等待时间和重试次数，断线及恢复会显示提示并写入日志。

签到处理中断后会重新查询服务器状态，再决定是否提交，避免直接重放结果未知的提交请求。监控、数字签到和雷达签到请求设置了 10 秒连接超时、30 秒读取超时；等待同学签到的查询保留 10 秒超时。启动时验证缓存会话、调用登录库抛出的暂时性网络异常也会重试，不会把验证时的断线误判为会话过期。登录库内部的超时和错误返回行为仍由该库控制。
